...
const actionAddRef = (field : string, ref : React.Ref<any>) : IValidationReducerAction => _actionValidationRule(VALIDATION_ACTIONS.addRef, field, ref)
...
const _fnCheckValidationField = <T>(state : IReducerValidationState<T>, key : string) : IValidationField => {
  ...
  if (!fieldRule.validation) {
    return {} as IValidationField
  }
...
const _fnValidateField = <T>(state : IReducerValidationState<T>, key : string, dirty : boolean = false) : IReducerValidationState<T> => {
  ...
  if (fieldRule.validation && !Object.keys(fieldRule.validation).every(x => x !== FUN_NAME_SAME_VALUES)) {
...
export const validationReducer = <T>() => {
  ...
  return (state : IReducerValidationState<T>, action : IValidationReducerAction) : IReducerValidationState<T> => {
    ...
    switch (action.type) {
      ...
      case VALIDATION_ACTIONS.addRef:
        return {
          ...state,
          refFields: [...state.refFields, {
            field: action.payload.field,
            ref: action.payload.data
          } as IFieldsRefs]
        }
      ...
      case VALIDATION_ACTIONS.removeDataArray:
        return validate((() => {
          ...
          const validation = (Array.isArray(arr) && arr.length > index) ? _.set({...(state.validation as any)}, `validations.${action.payload.field as string}`, [...arr.slice(0, index), ...arr.slice(index + 1, arr.length)]) :
            {...state.validation}
