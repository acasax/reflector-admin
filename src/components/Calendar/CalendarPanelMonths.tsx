    ...
    <>
      {
        array.map((arr, index) => {
          return (
            <div className={'calendar-data-rows'} key={`months_${index}`}>
              {
                arr.map((m, ind) => {
                  return (
                    <div
                                            className={`calendar-panel-data calendar-month${month === index * 3 + ind ? `${' active'}` : ''}`}
                                            key={`mon_${ind}`} calendar-data={index * 3 + ind}>
                      {m}
                    </div>
                  )
                })
